import { ComponentPropsWithoutRef } from 'react'

export const SvgIconInfo = (props: ComponentPropsWithoutRef<'svg'>) => (
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" {...props}>
    <path
      d="M9.82 18.64C12.2556 18.64 14.4606 17.6528 16.0567 16.0567C17.6528 14.4606 18.64 12.2556 18.64 9.82C18.64 7.38444 17.6528 5.17944 16.0567 3.58332C14.4606 1.98721 12.2556 1 9.82 1C7.38444 1 5.17944 1.98721 3.58332 3.58332C1.98721 5.17944 1 7.38444 1 9.82C1 12.2556 1.98721 14.4606 3.58332 16.0567C5.17944 17.6528 7.38444 18.64 9.82 18.64Z"
      stroke="white"
      strokeWidth="1.96"
      strokeLinejoin="round"
    />
    <path
      d="M10.5948 13.82C10.5948 14.248 10.2478 14.595 9.81982 14.595C9.39181 14.595 9.04482 14.248 9.04482 13.82C9.04482 13.392 9.39181 13.045 9.81982 13.045C10.2478 13.045 10.5948 13.392 10.5948 13.82Z"
      fill="white"
      stroke="white"
      strokeWidth="0.45"
    />
    <path
      d="M9.81982 5.82001L9.81982 10.82"
      stroke="white"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>
)
